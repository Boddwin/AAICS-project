"""
Experiment 03 — Random Forest speaker identification with logging.

Represents each utterance as the mean of its MFCC+delta frames and trains a
...
def train_rf(root_dir, dataset_name, logger=None):
    """Train Random Forest model."""
    if logger is None:
        logger = ExperimentLogger(f"rf_{dataset_name}")
    ...
    logger.info(f"Training Random Forest with 200 trees...")
...
def evaluate_rf(clf, le, root_dir, dataset_name, logger):
...
if __name__ == "__main__":
    ...
    clf, le, logger = train_rf(root_dir, dataset_name, logger=logger)
    if clf:
        evaluate_rf(clf, le, root_dir, dataset_name, logger)
        logger.info("RANDOM FOREST EXPERIMENT COMPLETE")
